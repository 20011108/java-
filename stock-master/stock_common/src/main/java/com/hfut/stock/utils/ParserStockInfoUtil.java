...
/*        Pattern compile = Pattern.compile("var S_Finance_bankuai_sinaindustry = (.+)");
        Matcher matcher = compile.matcher(jsData);
        List<StockBlockRtInfo> infos = new ArrayList<>();
        if(matcher.find()){
            String group = matcher.group(1);*/
        String jsonStr = stockStr.substring(stockStr.indexOf("=") + 1);
/*        ObjectMapper objectMapper = new ObjectMapper();
        HashMap<String,String> hashMap = objectMapper.readValue(group, HashMap.class);*/
